...
public class AdminDao {
    ...
    public static List getRoleActions(String loger) throws Exception {
        conn = DbConnector.getConnection();
        PreparedStatement stmt = null;
        ResultSet rs = null;
        List actionList = new ArrayList();
        try {
            stmt = conn.prepareStatement("Select * from role where role = (select aRole from admin where aName='" + loger + "')");
            rs = stmt.executeQuery();
            while (rs.next()) {
